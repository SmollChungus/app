...
type AnalogKeymapState = {
  rawDeviceMap: DeviceLayerMap;
  numberOfLayers: number;
  selectedLayerIndex: number;
  selectedKeys: number[] | null;
  analogKeyboardIsSelectable: boolean;
};

const initialAnalogState: AnalogKeymapState = {
  rawDeviceMap: {},
  numberOfLayers: 4,
  selectedLayerIndex: 0,
  selectedKeys: [],
  analogKeyboardIsSelectable: false,
};

const analogKeymapSlice = createSlice({
  name: 'analogKeymap',
  initialState: initialAnalogState,
  reducers: {
    ...
    setAnalogKeyboardIsSelectable: (
      ...
      state.analogKeyboardIsSelectable = action.payload;
    },
    loadLayerSuccess: (
      state,
    ...
    updateSelectedKeys: (state, action: PayloadAction<number[] | null>) => {
...
export const {
  ...
  saveKeymapSuccess,
  setAnalogKeyboardIsSelectable,
} = analogKeymapSlice.actions;

export default analogKeymapSlice.reducer;
